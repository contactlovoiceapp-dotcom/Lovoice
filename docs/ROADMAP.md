...
5. Rate limiting: per-user buckets in Postgres (`rate_limits` table) for `request_upload` (voice + message kinds), `like`, and `report`. Reasonable limits (e.g. 30 voice uploads/day, 100 likes/hour). Upload counters are consumed in `request_upload` only — not in `commit_upload`.
